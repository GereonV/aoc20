...
fn parse() -> [PasswordWithPolicy; INPUT_LINE_COUNT] {
	let mut buf: [MaybeUninit<PasswordWithPolicy>; INPUT_LINE_COUNT] = unsafe {
		MaybeUninit::uninit().assume_init()
	};
	let parse = |s: &str| unsafe { s.parse::<usize>().unwrap_unchecked() };
	for (l, p) in INPUT.lines().zip(&mut buf) {
		let (policy, password) = unsafe { l.split_once(": ").unwrap_unchecked() };
		let char = unsafe { policy.as_bytes().last().unwrap_unchecked() };
		let range = unsafe { policy.get_unchecked(..(policy.len() - 2)) };
		let (low, high) = unsafe { range.split_once('-').unwrap_unchecked() };
		let value = PasswordWithPolicy {
...
const INPUT: &str = r"5-6 s: zssmssbsms
3-6 j: jjjjjrrj
4-7 k: kfkgkkkkk
2-3 n: nkbgfnn
