...
unsafe fn index_behind_next_n_spaces(haystack: &str, mut index: usize, n: usize) -> usize {
	for _ in 0..n {
		let pos = haystack[index..].find(' ');
		let pos = unsafe { pos.unwrap_unchecked() };
		index += pos + 1;
	}
	index
}
...
impl<'a> Contents<'a> {
	fn new(list: &'a str) -> Option<Self> {
		match list.starts_with("no") {
			true => None,
			false => Some(Self { left: list })
		}
	}
}
...
impl<'a> Iterator for Contents<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let index = unsafe { index_behind_next_n_spaces(self.left, 0, 1) };
		let amt = self.left[..index - 1].parse();
		let amt = unsafe { amt.unwrap_unchecked() };
		self.left = &self.left[index..];
		let index = unsafe { index_behind_next_n_spaces(self.left, 0, 2) };
		let content = &self.left[..index - 1];
		self.left = &self.left[index..];
		self.left = match self.left.find(' ') {
			Some(idx) => &self.left[idx + 1..],
			None => "",
		};
		Some((amt, content))
	}
...
pub fn run() -> (usize, usize) {
	let mut map = HashMap::with_capacity(INPUT_LINES);
	let mut map2 = HashMap::with_capacity(INPUT_LINES);
	for line in INPUT.lines() {
		let index = unsafe { index_behind_next_n_spaces(line, 0, 2) };
		let container = &line[..index - 1];
		let index = unsafe { index_behind_next_n_spaces(line, index, 2) };
		let contained = map2.entry(container).or_insert(vec![]);
		let Some(contents) = Contents::new(&line[index..]) else {
			continue;
		};
	...
	while let Some(top) = stack.pop() {
		let Some(containers) = map.remove(top) else {
			continue;
		};
		stack.extend(containers.iter());
		set.extend(containers);
	}
	let mut stack = vec![(1, "shiny gold")];
	let mut count = 0;
	while let Some((multiplier, name)) = stack.pop() {
		for (amt, name) in map2[name].iter() {
			let mult = amt * multiplier;
			count += mult;
			stack.push((mult, name));
		}
	}
	(set.len(), count)
}

const INPUT_LINES: usize = 594;
const INPUT: &str = r"light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
