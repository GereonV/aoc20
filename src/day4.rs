pub fn run() -> (usize, usize) {
    ...
    for pw in INPUT.split("\n\n") {
            ...
            .filter(|pair| !pair.starts_with("cid"))
        {
            ...
            let (key, value) = unsafe { (pair.get_unchecked(..3), pair.get_unchecked(4..)) };
            // VALIDATORS.iter().find(fn(&&(&str, fn)) -> bool)
            if !VALIDATORS.iter().find(|&&(k, _)| k == key).unwrap().1(value) {
                valid = false;
            }
        }
        if amt_keys == VALIDATORS.len() {
            c1 += 1;
            if valid {
                c2 += 1;
            }
...
const VALIDATORS: [(&str, fn(&str) -> bool); 7] = [
